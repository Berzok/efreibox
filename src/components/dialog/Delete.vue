  ...
  <div v-show="this.visible" class="modal-background">
    <div class="niwa-modal-content d-flex flex-column">
      ...
      <div class="d-flex flex-fill justify-content-center">
        ...
        <button class="btn btn-primary btn-outline-danger w-50" @click="this.deleteItem">
...
export default defineComponent({
    ...
    computed: {
        deleteUrl() {
            if(this.item.type){
                return this.item.type === 'folder' ? '/folder/delete/' : '/resource/delete/'
            }
        }
    },
    data(){
    ...
    methods: {
        ...
        deleteItem() {
            if (this.item) {
                axios.delete(this.deleteUrl + this.item.id, {
                    headers: {
                        'X-Loading': true
                    }
                }).then((response, error) => {
